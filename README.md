...
6. In Paddle Sandbox, create one active monthly recurring Price for every Athreix tier, a client-side token, and a notification destination for `/api/billing/webhook`. Billing remains unavailable until the API key, destination secret, environment-matched client token, and all Price IDs pass readiness validation. For Live, use `NEXT_PUBLIC_PADDLE_ENV=production` with an entirely separate `pdl_live_...` API key, `live_...` client token, notification secret, and Live Price IDs; never reuse Sandbox values.
